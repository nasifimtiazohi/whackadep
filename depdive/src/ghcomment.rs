...
impl GitHubCommentGenerator {
    ...
    pub fn append_comment(&self, s: &str) {
        self.comment.borrow_mut().push_str(s);
    }

    pub fn add_text(&self, s: &str, style: &TextStyle) {
    ...
    pub fn add_newline(&self, count: u8) {
        for _i in 0..count {
            self.comment.borrow_mut().push('\n');
        }
    }

    pub fn add_bulleted_list<T: AsRef<str>>(&self, items: &[T], text_style: &TextStyle) {
    ...
    pub fn add_header(&self, s: &str, level: usize) {
    ...
    pub fn add_html_table<T: AsRef<str>>(&self, table: &[Vec<T>]) {
        self.add_newline(1);
...
mod test {
    ...
    fn test_ghcomment_mutliline() {
        let test_str = "rust";
        let gh = GitHubCommentGenerator::new();
